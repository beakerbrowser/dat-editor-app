/* globals Event editor DatArchive beaker alert confirm monaco localStorage */
...
async function getURLKey () {
  ...
  if (path === '/' || !path) return window.location.hostname // default to self, for now
  try {
...
function update () {
  ...
  // render header
  updateHeader(selectedArchive, selectedPath, activeUrl, isOwner, isEditable)
...
function rBlankScreen () {
  ...
  const onUpdate = () => alert('TODO') //beaker.archives.updateManifest(selectedArchive.url)
  return yo`
...
async function onNewFile (e) {
  ...
  window.history.pushState(null, '', `/${selectedArchive.info.key}/${path}`)
}
...
async function onOpenFile (e) {
  ...
  if (path !== selectedPath) {
    window.history.pushState(null, '', `/${archive.info.key}/${path}`)
  }
...
async function onRename (e) {
  ...
  if (selectedPath === e.detail.path) {
    window.history.pushState(null, '', `/${selectedArchive.info.key}/${targetPath}`)
  }
}
...
async function save () {
  ...
  if (newPath) {
    // go to file
    window.history.pushState(null, '', `/${selectedArchive.info.key}/${newPath}`)
  }
}

function closeModel (url) {
  url = url || selectedArchive.url + '/' + selectedModel.path
  var model = models[url]
  if (!model) return
